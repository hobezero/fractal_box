...
/// @todo Disable UBSan:unsigned-integer-overflow diagnostic (see Folly/hash/Hash.h)
template<c_hash_digest Digest>
inline constexpr
...
/// @brief Thomas Wang 64 bit mix hash function.
/// @todo Disable UBSan:unsigned-integer-overflow diagnostic (see Folly/hash/Hash.h)
inline constexpr
auto twang_mix64(uint64_t key) noexcept -> uint64_t {
...
/// @brief Inverse of twang_mix64.
inline constexpr
auto twang_unmix64(uint64_t key) noexcept -> uint64_t {
	key *= 4611686016279904257u;
	key ^= (key >> 28u) ^ (key >> 56u);
	key *= 14933078535860113213U;
	key ^= (key >> 14u) ^ (key >> 28u) ^ (key >> 42u) ^ (key >> 56u);
	key *= 15244667743933553977u;
	key ^= (key >> 24u) ^ (key >> 48u);
	key = (key + 1u) * 9223367638806167551u;
	return key;
}
